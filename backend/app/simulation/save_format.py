...
(`"0.2.0"`, no sector production), Phase 2B1 (`"0.3.0"`, no production-derived tax bases), and
Phase 2B2 (`"0.4.0"`, no derived labor allocation) saves are intentionally excluded: each ruleset
bump changes what turn resolution actually *does* and/or what `GameState` must contain (Phase 2A
added required player finance; Phase 2B1 added required player economy; Phase 2B2 removes
authored `tax_bases` in favor of required `tax_base_coefficients` and per-sector shares; Phase 2B3
removes authored `SectorState.employed_workers` in favor of a required
`EconomyState.effective_labor_force_share_bps`), and an older save has no recorded data to run the
...
and `docs/adr/0006-labor-allocation-at-fixed-prices.md`.
"""

SUPPORTED_CONTENT_VERSIONS: frozenset[str] = frozenset({"0.5.0"})
