...
class TestR1CrossReportValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            sorted(combo)
            for r in range(1, 11)
            for combo in itertools.combinations(_ELEVEN_REPORT_FIELDS, r)
        ],
    )
    def test_all_partial_combinations_of_eleven_reports_are_rejected(
        ...
        Phase 3B2A extends it again to eight; Phase 3B2B extends it again to nine; Phase 3C Gate
        3C1 extends it again to ten (`election`); Phase 3C Gate 3C2 extends it again to eleven
        (`coup_unrest`) — every proper nonempty subset of {labor_market, resources, production,
        tax_base_derivation, finance, political, legislative, political_capital,
        political_relationship, election, coup_unrest} (2,046 of them, up from 1,022) must be
        rejected.
        """
        data, _ = _valid_turn_report_dict()
        for field in _ELEVEN_REPORT_FIELDS:
    ...
    def test_all_five_absent_is_valid(self) -> None:
        ...
        Phase 3B1 extended it again to seven; Phase 3C Gate 3C1 extended it again to ten; Phase
        3C Gate 3C2 extended it again to eleven) — all must be nulled out here too, or this
