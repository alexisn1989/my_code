...
class TestR1CrossReportValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            sorted(combo)
            for r in range(1, 10)
            for combo in itertools.combinations(_TEN_REPORT_FIELDS, r)
        ],
    )
    def test_all_partial_combinations_of_ten_reports_are_rejected(
        ...
        3C1 extends it again to ten (`election`) — every proper nonempty subset of
        {labor_market, resources, production, tax_base_derivation, finance, political,
        legislative, political_capital, political_relationship, election} (1,022 of them, up from
        510) must be rejected.
        """
        data, _ = _valid_turn_report_dict()
        for field in _TEN_REPORT_FIELDS:
