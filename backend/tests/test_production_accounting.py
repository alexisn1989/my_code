...
class TestClassification:
    def test_capacity_constrained_when_labor_limited_output_exceeds_capacity(self) -> None:
        sector = _sector(quarterly_capacity_output=100, output_per_worker=10)
        result = compute_sector_output(sector, 20)
    ...
    def test_exactly_balanced_when_labor_limited_output_equals_capacity(self) -> None:
        sector = _sector(quarterly_capacity_output=200, output_per_worker=10)
        result = compute_sector_output(sector, 20)
        assert result.labor_limited_output == 200
        assert result.actual_output == 200
    ...
    def test_inactive_when_capacity_is_zero_and_no_employment(self) -> None:
        # R4/degenerate tie case: capacity == 0 AND allocated_workers == 0 is INACTIVE,
        ...
        result = compute_sector_output(sector, 0)
    ...
    def test_inactive_when_capacity_is_zero_even_with_employment(self) -> None:
        # capacity == 0 always wins regardless of allocated_workers/output_per_worker.
        sector = _sector(quarterly_capacity_output=0, output_per_worker=50)
        result = compute_sector_output(sector, 100)
        assert result.actual_output == 0
        assert result.constraint == SectorConstraint.INACTIVE
    ...
    def test_labor_constrained_not_inactive_when_capacity_positive_but_unstaffed(self) -> None:
        ...
        """
        sector = _sector(quarterly_capacity_output=500, output_per_worker=10)
        result = compute_sector_output(sector, 0)
        assert result.labor_limited_output == 0
        assert result.actual_output == 0
        assert result.capacity_utilization_bps == 0
...
class TestCapacityUtilizationBps:
    def test_floors_rather_than_rounds(self) -> None:
        ...
        sector = _sector(quarterly_capacity_output=3, output_per_worker=1)
        result = compute_sector_output(sector, 1)
        assert result.capacity_utilization_bps == 3333

    def test_is_10000_if_and_only_if_actual_output_equals_capacity(self) -> None:
        exact = _sector(quarterly_capacity_output=100, output_per_worker=10)
        short = _sector(quarterly_capacity_output=101, output_per_worker=10)
        assert compute_sector_output(exact, 10).capacity_utilization_bps == 10_000
        assert compute_sector_output(short, 10).capacity_utilization_bps != 10_000

    def test_zero_capacity_yields_zero_bps_not_a_division_error(self) -> None:
        sector = _sector(quarterly_capacity_output=0, output_per_worker=1)
        assert compute_sector_output(sector, 0).capacity_utilization_bps == 0

    def test_no_float_ever_appears_in_the_result(self) -> None:
        sector = _sector(quarterly_capacity_output=7, output_per_worker=3)
        result = compute_sector_output(sector, 5)
...
class TestAggregateProduction:
    def test_sums_employment_from_inputs_and_actual_output_from_results(self) -> None:
        ...
        allocated = (5, 20)
        results = tuple(
            compute_sector_output(s, w) for s, w in zip(sectors, allocated, strict=True)
        )
        aggregates = aggregate_production(allocated, results)
